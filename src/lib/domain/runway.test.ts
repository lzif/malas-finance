...
import type { RunwayInput } from './runway'
...
describe('computeRunway — the average window excludes today', () => {
  it('runway does not change when a transaction is saved today', () => {
    ...
    const before = computeRunway(base({ today, startedAt, dailySpendMap: mapWithoutToday, seedDailySpend: 0 }))
    const mapWithToday = { ...mapWithoutToday, [today]: 9_999_999 }
    const after = computeRunway(base({ today, startedAt, dailySpendMap: mapWithToday, seedDailySpend: 0 }))
